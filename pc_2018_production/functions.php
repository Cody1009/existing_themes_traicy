...
add_image_size('article-content-image-size', 600);
...
class kakakuAPI{
	...
	function daily_kakakuApi_action() {
      ...
      $lowest = $wifi_data['serviceList']['service'][0];
		  array_push( $options , array( $lowest['rentalFee'], $lowest['applyUrl'] ) );
		}
...
$date = new DateTime();
echo '<div id="traicy-talk-slider-container">';
    echo '<a class="talk-category-title" href="https://talk.traicy.com/" target="_blank" class="title">' . $title  . '</a>';
    echo '<ul id="thres">';
        foreach ($thres as $thre) :
        echo '<li class="thre mod">';
            echo '<a class="talk-title-text">' . $thre['title'] . '</a>';
